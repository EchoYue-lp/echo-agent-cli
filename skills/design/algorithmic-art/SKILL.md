...
1. Record the visual intent, canvas size, seed, parameters, and interaction model before coding.
2. Build the smallest deterministic prototype that demonstrates the chosen rule system.
3. Add controls only when they expose a meaningful artistic parameter; keep defaults useful without interaction.
4. Inspect a still frame and consecutive animation frames. Fix runaway particle counts, seams, aliasing, and inaccessible contrast before delivery.

## Delivery checklist

- Source runs from the documented command without network-only assets.
- A fixed seed reproduces the representative output.
- Exported output includes dimensions, frame rate or duration, and parameter values.
- The result is original and legible at the requested size.
