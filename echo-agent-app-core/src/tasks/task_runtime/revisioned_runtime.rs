//! EKO file persistence and product policy for the framework-owned task
//! revision service.
...
/// File-backed `RevisionedTaskStore` implementation. It deliberately has no
/// patch or validation logic; those remain authoritative in the framework
/// service.
pub struct EkoTaskGraphStore {
    blocking: TaskRuntimeOperation,
    pending_initial_runs: PendingInitialRuns,
    pending_direct_completions: PendingDirectCompletions,
}

impl EkoTaskGraphStore {
    pub fn new(store: Arc<TaskRuntimeStore>) -> Self {
        Self {
            blocking: TaskRuntimeOperation::new(store),
...
#[async_trait]
impl RevisionedTaskStore for EkoTaskGraphStore {
    async fn load(
        &self,
        scope_id: &str,
...
#[async_trait]
impl TaskToolPolicy for EkoTaskPolicy {
...
/// Apply an EKO task update through the framework patch protocol, then return
/// the existing EKO projection expected by GUI/TUI callers.
...
pub async fn commit_task_plan(
    ...
    let service = TaskRevisionService::new(
        Arc::new(EkoTaskGraphStore::new(store.clone())),
...
pub(crate) async fn commit_direct_completion(
    store: Arc<TaskRuntimeStore>,
    plan: super::types::TaskPlan,
    summary: TaskExecutionSummary,
    task_summary: String,
    ...
    let service = TaskRevisionService::new(
        Arc::new(EkoTaskGraphStore::with_pending_direct_completion(
            store.clone(),
            pending_direct_completions,
        )),
        Arc::new(DefaultTaskToolPolicy::new(run_id.clone())),
...
pub(crate) async fn publish_task_plan(
    store: Arc<TaskRuntimeStore>,
    run: TaskRun,
    trigger: InitialRunTriggerMetadata,
    continuation: Option<(bool, bool, Option<u64>, Option<u64>)>,
...
mod tests {
    ...
    async fn failed_initial_graph_validation_rolls_back_staged_run() -> Result<(), String> {
        ...
        let service = build_task_revision_service(store.clone(), test_capabilities());
    ...
    -> Result<(), String> {
        use super::super::types::{
            ExecutionMode, PlanTask, SubagentEvidence, SubagentEvidenceSource, SubagentOutcome,
            SubagentStatus, TaskExecutionSummary, TaskPlan, TaskRunStatus,
        };
        ...
        let mut result = SubagentOutcome::terminal(SubagentStatus::Completed, "done", Vec::new());
        ...
        let error =
            commit_direct_completion(store.clone(), plan, summary, "complete answer".to_string())
                .await
                .err()
    ...
    -> Result<(), String> {
        ...
        build_task_revision_service(restarted.clone(), test_capabilities())
            .create_from_tool(task_input("explorer"), &test_context("crash-run"))
            .await
            .map_err(|error| error.to_string())?;
        assert!(
