...
- No second execution-role terminology or framework-to-app dependency is introduced.
...
at least two real EKO consumers and a cycle-free boundary. This is an EKO
packaging decision only: it must not be used as a gate for product-neutral
capabilities entering the independent `echo-agent` framework. Without EKO
crate-extraction evidence, the single app-core package is the authoritative
deployment unit.
...
dependency-isolation conditions for EKO package extraction. This does not block
the framework from adding a standalone product-neutral primitive. R4 keeps one app-core package;
