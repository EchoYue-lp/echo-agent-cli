...
| Skill catalog and creator support | Complete / merge pending | Bundled `SKILL.md` files use only agentskills.io fields and description-driven routing. After contraction, `skill-creator` replaces `writing-skills` and `plugin-creator` is added: 25 bundled Skills, 7 enabled by default, with only `verification-before-completion` in the baseline. Three deferred model tools let both creators call the framework Skill validator and application PluginRuntime without parallel authorities; runtime builtin-root resolution, TUI/GUI activation, and Agent Plugins 1.0 installation remain shared. ADR [0033](./adr/0033-skill-catalog-contraction-and-official-frontmatter.md), [0036](./adr/0036-skill-policy-simplification.md), [0038](./adr/0038-builtin-creator-skills.md). |
| Unified turn-run binding | Feature branch / final gates pending | Every store-backed turn eagerly owns a TaskRun; typed provenance protects planless direct runs, boot and quiet-wake settlement are atomic, five user surfaces record steers against the exact run, and planless conversation runs stay out of the task UI. ADR [0037](./adr/0037-unified-turn-run-binding.md). |
