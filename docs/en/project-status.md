...
| Enabled Skill runtime authority | Complete / focused contract gates passed | `enabled-skills.json` filters bundled registration before descriptor/Hook insertion; primary and pooled IntentRouter projections use the active set, while SkillsHub remains catalog/install authority; behavior-level contract tests (disabled absent from every projection, reload takes effect, fail-closed on corrupt config, user skills unaffected, same-name precedence) and the catalog gate pass. ADR [0032](./adr/0032-enabled-skill-runtime-authority.md). |
| Skill catalog contraction and official standardization | Complete / static gates pending | Bundled `SKILL.md` files use agentskills.io official fields only (no private extension namespace), `allowed-tools` is a space-separated string, Skill files carry no private Hook sidecar, and LLM routing is description-driven; the catalog contracted 41 → 39 (`using-superpowers` removed, `deep-research` folded into `web-search`); framework `validate_skill_dir` plus the catalog gate test enforce zero violations and `BUILTIN_SKILL_NAMES` parity. ADR [0033](./adr/0033-skill-catalog-contraction-and-official-frontmatter.md). |
