...
| Conversation routing/inbox | `agent_router/`；不在 surface 解析地址或自建 mailbox |
...
| shared app services | `AppState`、`TaskRuntimeBlockingAdapter`、`ProductDataIoService`；GUI/headless 不得各建 authority；surface imports 走 `api/` |
