...
| Iteration 1: instruction system + ChatResources run_id | Complete | `instruction_provider.rs` renamed `.eko/AGENTS.md` → `.eko/learned-rules.md` with one-time best-effort migration (never clobbers an existing new file; falls back to legacy read on failure); legacy `ECHO_AGENT.md` / `~/.eko/instructions.md` / `~/.eko/AGENTS.md` entry points removed from `ProjectContext` (instructions now owned solely by `InstructionProvider`, eliminating duplicate `AGENTS.md` reads across `eko:instruction-context` and `eko:project-context` projections); `MEMORY.md` split into `get_memory_suffix()` separate from `get_instruction_suffix()` (still combined in projection — reversible, ready for a future split); `strip_yaml_frontmatter` tightened (requires closing `---` fence, rejects mid-doc horizontal rules, no longer silently strips partial bodies); `drive_chat` now propagates `formal_run_id` into `ExternalRunContext.run_id` + `EventIdentity.run_id` so task tools can read `run_id` from `ToolContext` (cross-spawn-safe) instead of falling back to the task_local scoped at `drive_chat:151`; verified 680 tests, clippy clean, fmt clean, GUI + no-default-features paths compile, frontend prettier+tsc clean |
| Iteration 2: webhook + HITL + config_watcher fixes | Complete | Webhook: removed dead `init_global`/`emit_global`/`global_emitter` singleton (was never initialized → 2 emit_global calls in CLI were silent no-ops); added `webhook_emitter: Option<Arc<WebhookEmitter>>` to `ReplConfig`, built from `AppConfig.webhooks.endpoints` in `modes.rs`; threaded into `chat_with_agent` so ChatCompleted + ToolFailed fire on the real emitter; `WebhookEmitter::emit` early-returns when no endpoints (avoids needless spawn); wired `CronTaskCompleted` emit into `scheduler/runner.rs::build_fire_fn` via a new emitter parameter (AppState shares its emitter with the scheduler). HITL: `HitlDispatcher::request` now clones the provider snapshot and drops the read guard immediately (previously held the lock across the up-to-5-minute wait, deadlocking concurrent register/unregister); switched from per-provider `timeout` to a single shared `timeout_at(deadline)` so the total wall-clock wait is strictly bounded. config_watcher: rewrote to watch the PARENT directory with a path filter (survives editor atomic-save+rename, which broke the old direct-file watch); resettable debounce drains the channel and resets the quiet window on each qualifying event (one save = one reload, not N serial reloads); module docs now state the hot-reload scope explicitly (hooks only — model/MCP/runtime limits still require restart). Verified 680 tests, clippy clean, fmt clean, GUI + no-default-features paths compile, frontend prettier+tsc clean |
| Iteration 3: migrate 3 file-backed storage impls down to framework | Complete | Framework (echo-agent commit `3c93b3a`): added `echo_state::memory::file_conversation::FileConversationStore` (no-dep `ConversationStore` impl over `<base>/conversations/<safe_id>.json` + `_meta.json` counter) and `echo_state::memory::conversation::{restore_message, restore_messages}` (inverse of `project_message`, returns `Result` instead of silently demoting unknown roles); added `echo_agent::state::file::FileRuntimeStateStore` (default-feature `RuntimeStateStore` impl over `<base>/runtime_state/<safe_id>/`). All three include robustness fixes vs the app's former private copies: corrupt JSON surfaces as a typed error (not silent empty/None — previously indistinguishable from "no data"); conversation ids sanitized to path-safe segments (rejects `/`, `\`, `..` — prevents directory escapes via `../foo`); atomic writes use uuid-suffixed temp files (no cross-write collisions). EKO cutover (this commit): replaced 3 `FileConversationStore` + 1 `FileRuntimeStateStore` + 3 `restore_messages` call sites with the framework paths; deleted app's `runtime_state_file.rs` and `conversation_restore.rs`; slimmed `conversation_file.rs` to retain only `SessionSearchEngine` (app-layer concern, not migrated) reading the framework's on-disk record shape; updated `runtime_state_e2e` test to import `echo_agent::state::FileRuntimeStateStore`. `SessionSearchEngine`, workspace routing, base-path ownership, and SQLite-disabled policy all stay in EKO. Verified framework 651 tests + strict panic/unwrap lint + no-default-features; EKO 676 tests, clippy clean, fmt clean, GUI + no-default-features paths compile, frontend prettier+tsc clean |
