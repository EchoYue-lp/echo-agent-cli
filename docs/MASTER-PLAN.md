...
| Long-horizon TaskRun continuation control plane (Phase C5) | Core complete; superseded follow-up planned as M0-M5 | One app-layer `TaskContinuationRuntime` owns idle continuation without a second graph/executor/store. Finite RunTurns have event-folded claim, exact driver settlement, token/time budgets, compaction accounting, Goal Contract/Recovery Capsule, blocker audit, cell wakeup, durable provider retry, typed boot admission, stable surface HITL replay, and cross-surface controls. Remaining evidence and performance work is governed by `docs/2026-08-16-eko-long-horizon-runtime-implementation-plan.md`; M1-M3 are complete. |
| Long-horizon runtime M0-M5 implementation | R0/M0/M1/M2/M3 complete; application M4 next | `docs/2026-08-16-eko-long-horizon-runtime-implementation-plan.md`; Runtime Goal active. App `de09946` makes `TaskRun.goal` the revision/hash-bound authority. Framework `cd4fccf` and app `9d59a0b` close M1. Framework `6d7d0cf` plus app `f4771f3` close M2 with exact-attempt controls through the existing `TurnSteerMailbox`. App `aa92178` closes M3 with event-folded provider retry, typed boot admission, exact orphan recovery, safe staged run creation, and GUI/TUI/CLI/channel parity. M4 must add the versioned Requirement/Evidence relation to the existing completion blocker path. |
...
R0/M0/M1/M2/M3 are complete. Application
`de09946`/`9d59a0b`/`f4771f3`/`aa92178` and framework `cd4fccf`/`6d7d0cf`
...
The active slice is application M4: add stable GoalRequirement IDs and versioned
Requirement-to-task/evidence coverage to the existing TaskRuntime completion
blocker path, invalidate affected evidence after Goal revision changes, and project
the same completion facts to GUI/TUI/CLI/channel without a second completion state
or `goal_complete` tool.
