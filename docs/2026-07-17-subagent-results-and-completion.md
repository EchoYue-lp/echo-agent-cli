...
- `WorkerReleased` 只保存 bounded summary，恢复后只能复用字符串，不能重新执行完成门禁。
...
- `WorkerReleased` 持久化 bounded structured result；resume 复用结果后仍重新执行相同门禁。
...
1. worker terminal status 是 `completed`；
2. summary 非空；
3. remaining_work 为空；
...
- process restart：按 execution id 读取 WorkerReleased structured result，重新执行 deterministic completion gate，不重新派发已经完整结束的 worker。
...
- worker timeout、cancel 和 failed 各自产生唯一且不同的 terminal status。
- 单 worker 失败、多 worker 部分成功、synthesis failure 和 resume-after-complete 均保留结构化结果与 unresolved failure。
- resume 复用 completed worker result，并重复相同门禁；不重复 worker 副作用。
