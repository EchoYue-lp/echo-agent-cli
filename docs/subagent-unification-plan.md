...
| 阶段 4:删 worker 协议和 bridge | 🟡 进行中(4a+4b 完成,4c 待续) | `1eb512b`(4a bridge 瘦身)+ `6ab3f31`(4b chat.rs 迁移 + 前端删旧监听,2026-07-03) | **4a**:bridge 瘦身(删 worker://trace 翻译 350 行,保留 execution://event)。**4b**:chat.rs main agent 路径迁到 execution://event(kind=\"run\" + kind=\"subagent\" subagent_run_id=\"main\"),前端删 worker://trace/subagent://event 监听 + normalizeWorkerTraceEvent。**4c 待续**:WorkerTraceEvent/Kind 类型 + WorkerTraceSink/trace_sink 链路仍在(executor 69处 + task_tools 25处 + execute_plan_tool 10处)。彻底删除需全链路改 TraceSink 签名 Fn(WorkerTraceEvent)→Fn(Value)。前端 workerTraceStore/subagentStore/生成代码待删 |
