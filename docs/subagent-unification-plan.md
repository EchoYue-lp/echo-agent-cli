...
| 阶段 4:删 worker 协议和 bridge | 🟡 进行中(4a 完成,4b 待续) | `1eb512b`(2026-07-03) | **4a 完成**:bridge 瘦身——删 worker://trace 翻译(350行)+ 本地 HashMap,保留 execution://event 双发(用事件自带 execution_id)。**4b 待续**(需协议设计决策):① chat.rs 的 main agent 路径事件(RunStarted/Thinking/Tool 等,38处)目前仍发 worker://trace,需迁移到 execution://event——但 main agent 不是 subagent,需扩展协议(kind="run" + main agent 执行流归属)② 删 WorkerTraceEvent/Kind 类型(8处)+ WorkerTraceSink/trace_sink 链路(executor 69处 + task_tools 25处 + execute_plan_tool 10处)③ 前端删 workerTraceStore/subagentStore + 重命名组件 Worker→Subagent ④ 删生成代码 WorkerTraceEvent.ts/Kind.ts |
