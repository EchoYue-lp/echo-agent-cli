...
| 阶段 4:删 worker 协议和 bridge | ✅ 完成 | `1eb512b`(4a bridge 瘦身)+ `6ab3f31`(4b chat.rs 迁移 + 前端删旧监听,2026-07-03)+ `46d2c1f`(4c 删 WorkerTrace 类型 + 改 TraceSink 签名,2026-07-03) | **4a**:bridge 瘦身(删 worker://trace 翻译 350 行,保留 execution://event)。**4b**:chat.rs main agent 路径迁到 execution://event(kind=\"run\" + kind=\"subagent\" subagent_run_id=\"main\"),前端删 worker://trace/subagent://event 监听 + normalizeWorkerTraceEvent。**4c**:彻底删 WorkerTraceEvent/WorkerTraceEventKind 类型 + WorkerTraceSink/trace_sink 链路。新增轻量 ExecEvent{run_id,task_id,event:&'static str,agent,payload} + ExecSink(Fn(ExecEvent)),executor 69 处 emit 改 emit_exec + 字符串字面量;task_tools TraceSink 改 Fn(ExecEvent) + 删 scoped_with_ctx_run_id 的 ctx.trace_sink 死回灌分支;chat_driver 删 on_worker_trace/trace_sink 方法;chat.rs sink 闭包改接收 ExecEvent;task_runtime.rs 两处 worker://trace 改 execution://event(kind=run);前端删 workerTraceStore/subagentStore + 生成代码 WorkerTraceEvent.ts/Kind.ts。关键发现:框架 ExternalRunContext.trace_sink 字段被注入但框架从不调用(subagent 走 SubagentEventBus)——死路径,worker_trace_sink_to_core 改恒返回 None。验证:cargo test 410 passed + GUI target + clippy 零错误 + 前端 tsc/build 全绿 |
