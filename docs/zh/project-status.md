...
| Skill catalog 与 creator 能力 | Complete / merge pending | 捆绑 `SKILL.md` 只用 agentskills.io 官方字段，LLM routing 为 description-driven；catalog 收敛后以 `skill-creator` 取代 `writing-skills` 并新增 `plugin-creator`，现为 25 个、默认启用 7 个、baseline 仍仅 `verification-before-completion`。三个 deferred 模型工具让 creator 直接调用 framework Skill validator 与应用 PluginRuntime，不建立平行 authority；builtin 根运行时解析，TUI/GUI 均可激活，install 识别 Agent Plugins 1.0 包。ADR [0033](./adr/0033-skill-catalog-contraction-and-official-frontmatter.md)、[0036](./adr/0036-skill-policy-simplification.md)、[0038](./adr/0038-builtin-creator-skills.md)。 |
