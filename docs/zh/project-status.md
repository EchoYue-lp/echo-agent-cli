...
| Enabled Skill 运行时权威 | Complete / focused contract gates passed | `enabled-skills.json` 在 descriptor/Hook 注册前过滤 bundled Skill；primary 与 pooled Agent 的 IntentRouter projection 只使用 active set，SkillsHub 继续负责 catalog/install；行为级契约测试（disabled 全投影缺席、reload 生效、fail-closed、user skill 不受误伤、同名优先级）与 catalog gate 已通过。ADR [0032](./adr/0032-enabled-skill-runtime-authority.md)。 |
| Skill catalog 收缩与官方标准化 | Complete / static gates pending | 捆绑 `SKILL.md` 只用 agentskills.io 官方字段（无任何私有扩展命名空间），`allowed-tools` 为空格分隔字符串，Skill 文件不携带私有 Hook sidecar，LLM routing 为 description-driven；catalog 41 → 39（删 `using-superpowers`、`deep-research` 并入 `web-search`）；framework `validate_skill_dir` + catalog gate 测试强制零违规与 `BUILTIN_SKILL_NAMES` 一致性。ADR [0033](./adr/0033-skill-catalog-contraction-and-official-frontmatter.md)。 |
