...
在 `echo-agent-app-core::agent_control` 增加一个 application routing service：
...
- TaskRuntime 提供 `RuntimeEventQuery { after_sequence, limit, execution_id, event_types }`
  ...
  占满 service query 的 limit。Subagent list 仅保留有界 attempt map，exact-attempt inspect/control
...
产品策略与 target discriminator 留在 application；service 只转换输入、注入 metadata、
...
`TaskRuntimeOperation` 离开 async executor。通用诊断/恢复调用仍可显式使用完整历史
