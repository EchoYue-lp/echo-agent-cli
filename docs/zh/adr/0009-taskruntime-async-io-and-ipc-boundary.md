...
executor 已通过进程共享、固定上限的 `TaskRuntimeOperation` 使用
...
3. 复用现有 bounded `TaskRuntimeOperation`，在 async 边界组合完整同步 transaction；
   IPC receipt 由 Rust DTO 和 ts-rs 生成。本项目采用此方案。

## 决策

- 所有 async production boundary 只通过 `TaskRuntimeOperation` 执行 journal/projection
...
- phase one 同时启动 framework command manager 的 shutdown owner，立即关闭 command admission 并
  取消长命令；phase two 才以 30 秒上限 join operation。CommandCell terminal 与 CommandCellWatch Ready
...
- ChatEventLog 的 CommandCellWatch Ready、GUI cancel 与 HITL orphan 投影通过 bounded product-data I/O
