...
- `LlmTimeouts` 的 non-stream request 与 stream first-chunk、idle、overall 边界；三种
  protocol 共用同一 SSE transport、取消、UTF-8 framing 与 EOF 校验。
...
`configured_models` 拥有模型 protocol、modality、sampling 与 context；`model_providers`
拥有 endpoint 与 credential。顶层 `model` 只保存 `default_model_id`，统一 typed resolver
不会回退到 mirror 或 synthetic model 字段。
...
当前 EKO 构造 `LlmConfig` 时直接使用 framework 的 typed timeout 默认值，不读取
`ECHO_AGENT_STREAM_*`，也不定义 timeout DTO 或 provider-local SSE loop。
