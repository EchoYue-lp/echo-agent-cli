...
TaskRuntime contained fewer PlanTask nodes. The old `task_execute({task})`
...
`task_create`. This produced three conflicting views: prose said N, the main
...
1. `task_execute` executes only the current persisted PlanTask DAG. The inline
   `task` parameter and implicit one-task fallback are deleted.
2. One `task_create` call materializes the complete initial PlanTask DAG and
   reports revision 1.
...
4. Planning calls `task_list` and passes the exact committed revision as
   `plan_revision`; later changes use `task_update(base_revision=...)`.
5. `task_execute` rejects empty plans and revision mismatches before dispatch.
...
7. The Agent must not claim dispatch before `task_execute` accepts the complete
   plan.
