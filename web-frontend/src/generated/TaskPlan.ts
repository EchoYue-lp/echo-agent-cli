// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { DomainProfile } from "./DomainProfile";
import type { ExecutionMode } from "./ExecutionMode";
import type { PlanTask } from "./PlanTask";

/**
 * A structured plan attached to a run. Generated by the planning runtime
 * (PR 2); never free-form markdown from the model.
 */
export type TaskPlan = { plan_id: string, run_id: string, domain_profile: DomainProfile, goal: string, assumptions: Array<string>, risks: Array<string>, execution_mode: ExecutionMode, tasks: Array<PlanTask>, };
