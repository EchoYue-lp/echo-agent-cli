...
import { Cpu, Wrench, Brain, GitBranch, Database, Server, Archive } from 'lucide-react';
...
function kindLabel(kind: TraceKind): string {
  switch (kind.type) {
    ...
    case 'agent_step':
      return `Step #${kind.step_number}`;
    case 'pipeline_stage':
      return `${kind.pipeline} / ${kind.stage}`;
    case 'memory_access':
      return `Memory: ${kind.operation}`;
    case 'mcp_call':
      return `MCP: ${kind.server}.${kind.method}`;
...
function kindColor(kind: TraceKind): string {
  switch (kind.type) {
    ...
    case 'agent_step':
      return 'var(--text-tertiary, #94a3b8)';
    case 'pipeline_stage':
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <span
                className="ml-auto text-[10px] font-mono whitespace-nowrap"
