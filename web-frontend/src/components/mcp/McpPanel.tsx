  ...
  return (
    <div className="p-3 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button onClick={loadData} className="rounded p-1 transition-colors"
      ...
      {saveMessage && (
        <div className="rounded-lg px-3 py-2 text-xs"
          style={{
            background: saveMessage.type === 'success' ? 'var(--color-success-bg)' : 'var(--color-error-bg)',
      ...
      <div className="space-y-2 rounded-lg border p-3" style={{ borderColor: s.border, background: s.bgHover }}>
        <div className="text-xs font-medium mb-1" style={{ color: s.text }}>MCP 配置（JSON 格式）</div>
        <div className="text-xs mb-2" style={{ color: s.textSec }}>编辑完整的 MCP 配置，保存后将立即应用。</div>
        ...
        {parseError && (
          <div className="rounded-lg px-2 py-1.5 text-xs" style={{ background: 'var(--color-error-bg)', color: 'var(--color-error)' }}>
            JSON 错误：{parseError}
          </div>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => {
            try {
              ...
              setParseError(null);
              setSaveMessage(null);
            } catch { setParseError('Invalid JSON - cannot format'); }
          }}
            className="flex-1 rounded-lg border py-1.5 text-xs transition-colors"
            style={{ borderColor: s.border, background: s.bg, color: s.text }}>
            格式化 JSON
      ...
      <div className="space-y-2">
        <div className="text-xs font-medium" style={{ color: s.text }}>已连接的服务器</div>
        {servers.length === 0 ? (
          <div className="rounded-lg px-3 py-4 text-center text-xs" style={{ color: s.textTer, background: s.bgHover }}>
            暂无 MCP 服务器连接
          </div>
        ) : (
          servers.map((srv) => {
                  ...
                  <div className="grid grid-cols-2 gap-1 text-xs">
                    <div>
                      ...
                      <span className="ml-2 font-medium"
                        style={{
                          color: srv.status === 'connected' ? 'var(--color-success)' : srv.status === 'error' ? 'var(--color-error)' : 'var(--color-warning)'
                        }}>
                        {srv.status}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="ml-2 font-medium" style={{ color: s.text }}>{srv.transport}</span>
                    </div>
                    {srv.connected_at && (
                      <div className="col-span-2">
                        <span style={{ color: s.textSec }}>连接时间：</span>
                        <span className="ml-2" style={{ color: s.text }}>{new Date(srv.connected_at).toLocaleString()}</span>
                      </div>
                    )}
                    {srv.error && (
                      <div className="col-span-2">
                        <span style={{ color: s.textSec }}>错误：</span>
                        <span className="ml-2" style={{ color: 'var(--color-error)' }}>{srv.error}</span>
                      </div>
                    )}
                  </div>
