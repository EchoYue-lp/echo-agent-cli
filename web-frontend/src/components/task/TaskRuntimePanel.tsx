...
import { RuntimeStoryCard } from './RuntimeStoryCard';
import { PlanEditor } from './PlanEditor';
import { ResultFullView } from './ResultFullView';
      ...
      {expanded && (
        <div className="mt-3 space-y-3 border-l pl-3" style={{ borderColor: 'var(--border-primary)' }}>
          ...
          {worker.task && (
            ...
            >
              <MarkdownContent content={worker.task} maxHeight={288} className="text-[11px]" />
            </TraceSection>
          )}
          ...
          {result && (
            ...
            >
              <MarkdownContent content={result} maxHeight={520} className="text-[11px]" />
            </TraceSection>
          )}
          ...
          {(thinking || tools.length > 0) && (
            ...
            >
              {thinking && (
                <MarkdownContent content={thinking} maxHeight={288} className="mb-2 text-[11px]" />
              )}
              {tools.length > 0 && (
...
export function TaskRuntimeMainPanel() {
  ...
  const [editingPlan, setEditingPlan] = useState<Array<{ id: string; title: string; description?: string; status?: string }> | null>(null);
  ...
  const isInferred = effectiveRouteExplanation?.routeReason?.includes('实时 plan_ready 路由事件不可用');
  ...
  return (
    <>
      ...
      <RuntimeStoryCard
        title="任务执行"
        status={['completed'].includes(activeRun.status) ? 'done' : ['failed', 'cancelled'].includes(activeRun.status) ? 'error' : activeRun.status === 'running' ? 'active' : 'pending'}
        meta={<span className="rounded px-1.5 py-0.5 text-[10px] font-medium" style={{ color: statusColor(activeRun.status), background: 'var(--bg-hover)' }}>{STATUS_LABEL[activeRun.status] ?? activeRun.status}</span>}
        ...
        >
          ...
          >
            ...
            <div className="flex gap-1.5">
              ...
              <button
                onClick={() => setEditingPlan(plan.tasks as Array<{ id: string; title: string; description?: string; status?: string }>)}
      ...
      {visibleTraceWorkers.length > 0 ? (
        <RuntimeStoryCard
          title="并行执行"
          status={visibleTraceWorkers.some(w => w.status === 'running') ? 'active' : 'done'}
      ...
      {finalResult && (
        ...
        >
          ...
          >
            ...
            <MarkdownContent content={finalResult} maxHeight={720} className="text-[12px]" />
          </div>
        </RuntimeStoryCard>
      )}
        ...
        <PlanEditor
          ...
          onSave={async (json) => {
            try {
              ...
              await taskRuntimeApi.editPlan(activeRun.run_id, tasks);
              refresh(activeRun.run_id);
      ...
      {showFullResult && finalResult && (
        <ResultFullView
          content={finalResult}
          onClose={() => setShowFullResult(false)}
        />
      )}
    </>
  );
