    ...
    <div className="p-3 space-y-3">
        ...
        {modes.map((m) => (
          ...
          >
            ...
            <div>
              ...
              <span className="ml-2 text-xs" style={{ color: s.textTer }}>
                {m.desc}
              </span>
            </div>
          </label>
      ...
      {showAdd && (
        <div
          ...
          style={{ borderColor: s.border, background: s.accentBg }}
        >
          <input
            value={addForm.name}
          ...
          <select
            ...
            onChange={(e) =>
              setAddForm({ ...addForm, effect: e.target.value as 'allow' | 'deny' | 'ask' })
            }
            className="w-full rounded-lg border px-2 py-1.5 text-xs"
            style={{ background: s.bgInput, borderColor: s.border, color: s.text }}
          >
      ...
      {rules.map((rule) => (
        ...
        >
          ...
          <StatusBadge
            status={
              rule.effect === 'allow' ? 'success' : rule.effect === 'deny' ? 'error' : 'warning'
            }
