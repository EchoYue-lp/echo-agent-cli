...
import type {
  ...
  MemoryEntry, NamespacesResponse, SnapshotInfo, ConfigInfo,
...
export const sessionApi = {
  get: () => isTauri()
    ? apiInvoke<SessionInfo>('get_session')
    : get<SessionInfo>('/session'),
  reset: () => isTauri()
    ? apiInvoke<SessionInfo>('reset_session')
    : post<SessionInfo>('/session/reset'),
  getLatest: () => isTauri()
    ? apiInvoke<{ found: boolean; id?: string; title?: string; updated_at?: string; message_count?: number; error?: string }>('get_latest_session')
    : get<{ found: boolean; id?: string; title?: string; updated_at?: string; message_count?: number; error?: string }>('/session/latest'),

  createCheckpoint: () => isTauri()
    ? apiInvoke<{ success: boolean; snapshot_id?: string }>('create_checkpoint')
    : post<{ success: boolean; snapshot_id?: string }>('/session/checkpoint'),
  listCheckpoints: () => isTauri()
    ? apiInvoke<SnapshotInfo[]>('list_checkpoints')
    : get<SnapshotInfo[]>('/session/checkpoints'),
  restoreCheckpoint: (id: string) => isTauri()
    ? apiInvoke<{ success: boolean; restored_to?: string }>('restore_checkpoint', { snapshot_id: id })
    : post<{ success: boolean; restored_to?: string }>(`/session/restore/${id}`),
};

export const historyApi = {
  getHistory: () => isTauri()
    ? apiInvoke<HistoryResponse>('get_history')
    : get<HistoryResponse>('/history'),
  exportMarkdown: () => isTauri()
    ? apiInvoke<{ format: string; content: string; message_count: number }>('export_history_markdown')
    : get<{ format: string; content: string; message_count: number }>('/history/export?format=markdown'),
  exportJson: () => isTauri()
    ? apiInvoke<{ format: string; content: string; message_count: number }>('export_history_json')
    : get<{ format: string; content: string; message_count: number }>('/history/export?format=json'),
};

export const toolsApi = {
  list: () => isTauri()
    ? apiInvoke<ToolInfo[]>('list_tools')
    : get<ToolInfo[]>('/tools'),
  get: (name: string) => isTauri()
    ? apiInvoke<ToolInfo>('get_tool', { name })
    : get<ToolInfo>(`/tools/${name}`),
  enable: (name: string) => isTauri()
    ? apiInvoke<{ success: boolean }>('enable_tool', { name })
    : post<{ success: boolean }>(`/tools/${name}/enable`),
  disable: (name: string) => isTauri()
...
export const skillsApi = {
  list: () => isTauri()
    ? apiInvoke<SkillInfo[]>('list_skills')
    : get<SkillInfo[]>('/skills'),
  get: (name: string) => isTauri()
    ? apiInvoke<SkillInfo>('get_skill', { name })
    : get<SkillInfo>(`/skills/${name}`),
  load: (dir: string) => isTauri()
    ? apiInvoke<{ success: boolean }>('load_skill', { name: dir })
    : post<{ success: boolean }>('/skills/load', { dir }),
  upload: (rootDir: string, files: { path: string; content: string }[]) => isTauri()
...
export const mcpApi = {
  list: () => isTauri()
    ? apiInvoke<McpServerInfo[]>('list_mcp_servers')
    : get<McpServerInfo[]>('/mcp'),
  get: (name: string) => isTauri()
    ? apiInvoke<McpServerInfo>('get_mcp_server', { name })
    : get<McpServerInfo>(`/mcp/${name}`),
  connect: (req: { name: string; transport: { transport: string;[key: string]: unknown } }) => isTauri()
    ? apiInvoke<{ success: boolean; name?: string; error?: string }>('connect_mcp_server', { name: req.name, transport: req.transport })
    : post<McpServerInfo>('/mcp/connect', req),
  disconnect: (name: string) => isTauri()
    ? apiInvoke<{ success: boolean }>('disconnect_mcp_server', { name })
    : post<{ success: boolean }>(`/mcp/${name}/disconnect`),
  toggle: (name: string, enabled: boolean) => isTauri()
    ? apiInvoke<{ success: boolean; enabled: boolean; message?: string }>('toggle_mcp_server', { name, enabled })
    : post<{ success: boolean; enabled: boolean; message?: string }>(`/mcp/${name}/toggle`, { enabled }),
  getConfig: () => isTauri()
    ? apiInvoke<McpConfig>('get_mcp_config')
    : get<McpConfig>('/mcp/config'),
  updateConfig: (config: McpConfig) => isTauri()
...
export const configApi = {
  get: () => isTauri()
    ? apiInvoke<ConfigInfo>('get_config')
    : get<ConfigInfo>('/config'),
  update: (cfg: Partial<ConfigInfo>) => isTauri()
  ...
  updateFull: (cfg: Partial<FullConfigUpdateRequest>, _signal?: AbortSignal) => isTauri()
...
export const permissionsApi = {
  ...
  addRule: (rule: Omit<PermissionRule, 'priority'>) => isTauri()
    ? apiInvoke<PermissionRule>('add_permission_rule', { matcher: rule.name, behavior: rule.effect, source: 'manual' })
    : post<PermissionRule>('/permissions/rules', rule),
  removeRule: (name: string) => isTauri()
    ? apiInvoke<{ success: boolean }>('remove_permission_rule', { matcher: name })
    : del<{ success: boolean }>(`/permissions/rules/${name}`),
};

export const auditApi = {
  logs: (offset?: number, limit?: number) => isTauri()
    ? apiInvoke<{ logs: AuditLog[]; total: number; offset: number; limit: number }>('get_audit_logs', { offset, limit })
    : get<{ logs: AuditLog[]; total: number; offset: number; limit: number }>(`/audit/logs${offset !== undefined ? `?offset=${offset}&limit=${limit ?? 100}` : ''}`),
  stats: () => isTauri()
    ? apiInvoke<{ total: number; allowed: number; denied: number; asked: number }>('get_audit_stats')
...
export const sandboxApi = {
  ...
  updateConfig: (cfg: SandboxConfig) => isTauri()
    ? apiInvoke<SandboxConfig>('update_sandbox_config', { config: cfg })
    : put<SandboxConfig>('/sandbox/config', cfg),
  execute: (req: SandboxExecuteRequest) => isTauri()
    ? apiInvoke<SandboxExecuteResult>('execute_sandbox', { code: req.code, language: req.language })
    : post<SandboxExecuteResult>('/sandbox/execute', req),
};

export const compressApi = {
  trigger: (options?: { keep_messages?: number }) => isTauri()
...
export const conversationApi = {
  ...
  save: (data: { id: string; title: string; messages: SavedMessage[]; model?: string }) => isTauri()
    ? apiInvoke<{ success: boolean; id: string }>('save_conversation', { id: data.id, title: data.title, messages: data.messages })
  ...
  update: (id: string, data: { title?: string; messages?: SavedMessage[] }) => isTauri()
    ? apiInvoke<{ success: boolean }>('update_conversation', { id, title: data.title, messages: data.messages })
  ...
  restore: (id: string) => isTauri()
    ? apiInvoke<{ success: boolean; message_count: number; conversation_id: string }>('restore_conversation', { id })
    : post<{ success: boolean; message_count: number; conversation_id: string }>(`/conversations/${id}/restore`),
};

export const contextApi = {
  get: () => isTauri()
    ? apiInvoke<ContextStats>('get_context_stats')
    : get<ContextStats>('/context'),
};
...
export const tasksApi = {
  list: () => isTauri()
    ? apiInvoke<BackgroundTask[]>('list_tasks')
    : get<BackgroundTask[]>('/tasks'),
  get: (id: string) => isTauri()
    ? apiInvoke<BackgroundTask>('get_task', { id })
    : get<BackgroundTask>(`/tasks/${id}`),
  submit: (req: SubmitTaskRequest) => isTauri()
    ? apiInvoke<{ success: boolean; task_id: string }>('submit_task', { kind: req.kind, description: req.description, params: req.params })
    : post<{ success: boolean; task_id: string }>('/tasks', req),
  cancel: (id: string) => isTauri()
...
export const papersApi = {
  list: (params?: { tag?: string; search?: string }) => isTauri()
  ...
  get: (id: string) => isTauri()
    ? apiInvoke<Paper>('get_paper', { id })
    : get<Paper>(`/papers/${id}`),
  create: (req: CreatePaperRequest) => isTauri()
  ...
  updateNotes: (id: string, notes: string) => isTauri()
    ? apiInvoke<Paper>('update_paper_notes', { id, notes })
    : put<Paper>(`/papers/${id}/notes`, { notes }),
  addTags: (id: string, tags: string[]) => isTauri()
...
export const workspaceApi = {
  ...
  create: (name: string, kind?: string, root?: string) => isTauri()
    ? apiInvoke<{ success: boolean; workspace: Workspace }>('create_workspace', { name, kind, root })
  ...
  linkProject: (id: string, path: string) => isTauri()
  ...
  defaultRoot: (name: string) => isTauri()
...
export const evolutionApi = {
  trajectories: (date?: string) => isTauri()
  ...
  trajectoryStats: () => isTauri()
    ? apiInvoke<{ stats: TrajectoryStats }>('get_trajectory_stats')
    : get<{ stats: TrajectoryStats }>('/evolution/trajectories/stats'),
  review: (runId?: string) => isTauri()
    ? apiInvoke<{ success: boolean; run_id: string; actions: string[]; nothing_to_save: boolean; error?: string | null }>('review_trajectory', { trajectory_id: runId })
    : post<{ success: boolean; run_id: string; actions: string[]; nothing_to_save: boolean; error?: string | null }>(
        '/evolution/review',
        { run_id: runId }
      ),
  curator: (action: string, skillName?: string) => isTauri()
    ? apiInvoke<{
        success: boolean;
...
export const providerApi = {
  ...
  test: (req: { provider: string; model: string; api_key?: string; base_url?: string }) => isTauri()
    ? apiInvoke<TestConnectionResponse>('test_connection', req)
    : post<TestConnectionResponse>('/providers/test', req),
  switch: (req: { model: string; api_key?: string; base_url?: string; provider?: string; temperature?: number; max_tokens?: number }) => isTauri()
...
export const pluginApi = {
  list: () => isTauri()
    ? apiInvoke<PluginInfo[]>('list_plugins')
    : get<PluginInfo[]>('/plugins'),
  get: (name: string) => isTauri()
  ...
  install: (req: { source: string; scope?: string }) => isTauri()
    ? apiInvoke<{ success: boolean; plugin_id?: string; info?: PluginInfo; error?: string }>('install_plugin', { source: req.source, scope: req.scope })
    : post<{ success: boolean; plugin_id?: string; info?: PluginInfo; error?: string }>('/plugins/install', req),
  uninstall: (req: { name: string; keep_data?: boolean }) => isTauri()
    ? apiInvoke<{ success: boolean; message?: string; error?: string }>('uninstall_plugin', { name: req.name, keep_data: req.keep_data })
  ...
  disable: (name: string) => isTauri()
    ? apiInvoke<{ success: boolean; message?: string; error?: string }>('disable_plugin', { name })
    : post<{ success: boolean; message?: string; error?: string }>(`/plugins/${name}/disable`),
  reload: () => isTauri()
    ? apiInvoke<{ success: boolean; total?: number; enabled?: number; message?: string; error?: string }>('reload_plugins')
...
export const schedulerApi = {
  ...
  create: (data: { name: string; cron_expr: string; prompt: string }) => isTauri()
    ? apiInvoke<{ success: boolean }>('add_scheduler_task', { name: data.name, cron_expr: data.cron_expr, prompt: data.prompt })
    : post<{ success: boolean }>('/scheduler/tasks', data),
  updateStatus: (id: string, enabled: boolean) => isTauri()
    ? apiInvoke<{ success: boolean }>('set_scheduler_task_status', { id, status: enabled ? 'enabled' : 'disabled' })
    : put<{ success: boolean }>(`/scheduler/tasks/${id}/status`, { status: enabled ? 'enabled' : 'disabled' }),
  run: (id: string) => isTauri()
    ? apiInvoke<{ success: boolean; result?: string; error?: string }>('run_scheduler_task', { id })
...
export const humanGateApi = {
  ...
  respond: (taskId: string, selection: string, instructions?: string) => isTauri()
    ? apiInvoke<{ success: boolean }>('respond_human_gate', { gate_id: taskId, response: selection, instructions })
...
export const worktreeApi = {
  list: () => isTauri()
    ? apiInvoke<WorktreeInfo[]>('list_worktrees')
    : get<WorktreeInfo[]>('/worktrees'),
  create: (req: { branch: string; base?: string }) => isTauri()
    ? apiInvoke<WorktreeInfo>('create_worktree', { branch: req.branch, path: req.base })
    : post<WorktreeInfo>('/worktrees', req),
  remove: (branch: string) => isTauri()
