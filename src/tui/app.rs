...
impl TuiApp {
    async fn new(agent: AgentHandle) -> Self {
        ...
        // 收集 agent 元数据
        let (mcp_servers, skills, tools) = agent.read(|a| {
            let mcp: Vec<String> = a.mcp_server_names().into_iter().map(|s| s.to_string()).collect();
            let skills: Vec<String> = a.skill_names().into_iter().map(|s| s.to_string()).collect();
            let tools: Vec<String> = a.tool_names().into_iter().map(|s| s.to_string()).collect();
            (mcp, skills, tools)
        }).await;
    ...
    /// 启动事件循环
    async fn run(&mut self, terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) -> anyhow::Result<()> {
        // 欢迎消息
        ...
        match event {
            ...
            AgentEvent::ThinkEnd {
                ...
                self.status.completion_tokens =
                    self.status.completion_tokens.saturating_add(completion_tokens);
            }
