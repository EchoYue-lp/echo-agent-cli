...
pub async fn list_mcp_servers(
    ...
    let servers: Vec<McpServerInfo> = state.connection.agent.read(|agent| {
        agent
            .list_mcp_servers()
            .into_iter()
            .filter_map(|name| {
                    ...
                    .map(|hs| {
                        if hs.healthy {
                            ...
                            (McpConnectionStatus::Error(
                                hs.error.clone().unwrap_or_else(|| "Unknown error".to_string())
                            ), hs.error.clone())
                        }
                    })
...
fn validate_mcp_stdio_command(command: &str, args: &[String]) -> Result<(), WebError> {
    ...
    // ── 参数校验 ──────────────────────────────────────────────────────────
    if args.len() > 50 {
        return Err(WebError::Validation("Too many arguments (max 50)".to_string()));
    }
...
pub async fn connect_mcp_server(
    ...
    let config = match &req.transport {
        McpTransportConfig::Stdio { command, args, env } => {
            ...
            McpServerConfig::stdio_with_env(req.name.as_str(), command.as_str(), args_clone, env_vec)
        }
    ...
    match state.connection.agent.write_async(|agent| Box::pin(async move {
        agent.connect_mcp_from_config(config).await
    })).await {
        Ok(_client) => {
            // 获取实际的工具信息
            let tools: Vec<McpToolInfo> = state.connection.agent.read(|agent| {
                agent.mcp_client(&server_name)
                    .map(|client| {
                        client.tools().iter().map(mcp_tool_to_info).collect()
                    })
                    .unwrap_or_default()
...
pub async fn get_mcp_server(
    ...
    let tools: Vec<McpToolInfo> = match state.connection.agent.read(|agent| {
        agent.mcp_client(&name)
...
pub async fn disconnect_mcp_server(
    ...
    // 检查服务端是否存在
    let exists = state.connection.agent.read(|agent| agent.mcp_client(&name).is_some()).await;
    if !exists {
    ...
    let disconnected = state.connection.agent.write_async(|agent| Box::pin(async move {
        agent.disconnect_mcp(&name_for_disconnect).await
    })).await;

    if disconnected {
        // 更新健康状态为已断开
        let mut health = state.plugins.mcp_health.write().await;
        health.insert(name.clone(), crate::state::McpHealthStatus {
            name: name.clone(),
            healthy: false,
...
pub async fn update_mcp_config(
    ...
    match config.to_server_configs() {
        Ok(server_configs) => {
            ...
            let (errors, connected_servers) = state.connection.agent.write_async(|agent| Box::pin(async move {
                // 断开所有现有 MCP 连接
                let existing_servers: Vec<String> = agent.list_mcp_servers().into_iter().map(|s| s.to_string()).collect();
        ...
        Err(e) => {
            WebError::Validation(format!("无效的 MCP 配置: {}", e)).into_response()
        }
    }
}