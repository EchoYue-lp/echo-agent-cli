...
use crate::state::{AppState, StoredWorkflow, WorkflowDefinition};
...
pub async fn create_workflow(
    ...
    // 解析 JSON 定义
    let def: WorkflowDefinition = match serde_json::from_str(&req.definition) {
        Ok(d) => d,
        Err(e) => {
    ...
    let edge_count = if node_count > 1 { node_count - 1 } else { 0 };
    ...
    let mut workflows = state.workflows.write().unwrap();
...
pub async fn execute_workflow(
    ...
    let def = {
        ...
        let w = workflows
            .get(&id)
            .ok_or_else(|| AppError::NotFound(format!("工作流 '{}' 未找到", id)))?;
        let parsed: WorkflowDefinition = serde_json::from_str(&w.definition)
            .map_err(|e| AppError::Internal(format!("工作流定义解析失败: {}", e)))?;
        parsed
    };

    // 执行工作流步骤（线性执行 prompt 步骤，记录 tool 步骤信息）
