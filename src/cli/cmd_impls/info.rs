...
cmd!(ToolsCommand, "tools", ["t"], CommandCategory::Info, "List registered tools", cmd_tools);
...
cmd!(CostCommand, "cost", CommandCategory::Info, "Show session cost estimate", cmd_cost);
...
async fn cmd_usage(ctx: &CommandContext, _: &[&str]) -> CommandOutcome {
    ...
    let (in_price, out_price, currency) = estimate_price(&model);
    let total = (input_tokens as f64 / 1000.0) * in_price + (output_tokens as f64 / 1000.0) * out_price;
...
cmd!(UsageCommand, "usage", CommandCategory::Info, "Show token usage and cost", cmd_usage);
...
cmd!(DebugCommand, "debug", ["dbg"], CommandCategory::Debug, "Show debug information", cmd_debug);
