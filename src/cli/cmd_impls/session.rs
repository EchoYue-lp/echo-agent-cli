...
async fn cmd_reset(ctx: &CommandContext, _: &[&str]) -> CommandOutcome {
    ctx.agent.read_async(|a| Box::pin(async move {
        ...
        ctx.clear();
        ctx.push(echo_agent::llm::types::Message::system("You are a helpful assistant.".to_string()));
    })).await;
    println!("Conversation reset.");
    CommandOutcome::Continue
}
cmd!(ResetCommand, "reset", ["r"], CommandCategory::Session, "Reset conversation history", cmd_reset);
...
async fn cmd_history(ctx: &CommandContext, _: &[&str]) -> CommandOutcome {
    ctx.agent.read_async(|a| Box::pin(async move {
        ...
        for (i, m) in msgs.iter().enumerate() {
            let preview: String = m.content.as_text().unwrap_or_default().chars().take(80).collect();
            println!("  {i:3} [{}] {preview}", m.role.as_str());
        }
    })).await;
    CommandOutcome::Continue
}
cmd!(HistoryCommand, "history", ["hist"], CommandCategory::Session, "View conversation history", cmd_history);
...
cmd!(StatsCommand, "stats", ["st"], CommandCategory::Session, "Show context statistics", cmd_stats);
...
cmd!(StatusCommand, "status", CommandCategory::Session, "Show agent runtime status", cmd_status);
...
cmd!(NewCommand, "new", ["n"], CommandCategory::Session, "Start new session", cmd_new);
...
cmd!(UndoCommand, "undo", ["u"], CommandCategory::Session, "Undo last assistant message", cmd_undo);
