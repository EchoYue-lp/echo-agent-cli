...
impl CommandHandler {
    ...
    async fn cmd_reset(&self) {
        self.agent.write_async(|a| Box::pin(async move { a.reset().await })).await;
        println!("\n✅ 对话已重置");
    }
    ...
    async fn cmd_skills_list(&self) {
        let skills: Vec<String> = self.agent.read(|a| a.skill_names().iter().map(|s| s.to_string()).collect()).await;

        println!("\n╭─────────────────────────────────────────────────────────────╮");
        println!("│                    🎯 已加载技能 ({} 个)                       │", skills.len());
        println!("╰─────────────────────────────────────────────────────────────╯");
        ...
        if !unloaded.is_empty() {
            println!();
            println!("╭─────────────────────────────────────────────────────────────╮");
            println!("│                    📦 Hub 可用技能 ({} 个未加载)               │", unloaded.len());
            println!("╰─────────────────────────────────────────────────────────────╯");
            for e in &unloaded {
    ...
    async fn cmd_skills_search(&self, query: &str) {
        ...
        println!("\n╭─────────────────────────────────────────────────────────────╮");
        println!("│                    🔍 技能搜索: \"{}\" ({} 个结果)             │", query, results.len());
        println!("╰─────────────────────────────────────────────────────────────╯");
        ...
        if results.is_empty() {
            ...
            for e in &results {
                let status = if e.loaded { "✓ 已加载" } else { "○ 未加载" };
    ...
    async fn cmd_skills_install(&self, args: &str) {
        ...
        let result = if args.starts_with("http://") || args.starts_with("https://") || args.ends_with(".git") {
    ...
    async fn cmd_skills_info(&self, name: &str) {
        ...
        match hub.get(name) {
            Some(e) => {
                ...
                println!("  状态:       {}", if e.loaded { "已加载" } else { "未加载" });
            }
            None => println!("\n  技能 '{}' 未在 Hub 中找到", name),
        }
        println!();
    ...
    async fn cmd_mcp(&self) {
        let servers: Vec<String> = self.agent.read(|a| a.mcp_server_names().iter().map(|s| s.to_string()).collect()).await;
    ...
    async fn cmd_history(&self) {
        let messages = self.agent.read_async(|a| Box::pin(async move { a.get_messages().await })).await;
        ...
        for (i, msg) in messages.iter().enumerate() {
            ...
            println!("  {}. {} {}: {}{}", i + 1, role_icon, msg.role.as_str(), preview, suffix);
    ...
    async fn cmd_compress(&self) {
        ...
        match self.agent.read_async(|a| Box::pin(async move {
            a.force_compress_with(&compressor).await
        })).await {
            Ok(stats) => {
                println!("\n✅ 压缩完成:");
    ...
    async fn cmd_stats(&self) {
        let (msg_count, tokens, token_limit) = self.agent.read_async(|a| Box::pin(async move {
    ...
    async fn cmd_system(&self, args: &[&str]) {
        ...
        self.agent.write_async(|a| Box::pin(async move { a.set_system_prompt(prompt.clone()).await })).await;
        println!("\n✅ 系统提示词已更新");
    }
    ...
    async fn cmd_save(&self, args: &[&str]) {
        ...
        let (messages, model, system_prompt) = self.agent.read_async(|a| Box::pin(async move {
    ...
    async fn cmd_load(&self, args: &[&str]) {
        ...
        match persistence.load_session(name) {
            Ok(session) => {
                ...
                self.agent.write_async(|a| Box::pin(async move { a.load_messages(messages).await })).await;
    ...
    async fn cmd_theme(&self, args: &[&str]) {
        if args.is_empty() {
            ...
            self.output.print_info(&format!("当前主题: {}", current.name));
            self.output.print_info("可用主题: dark, light, monokai, solarized, dracula, one-dark");
        ...
        match ColorTheme::from_name(name) {
            Some(theme) => {
                ...
                self.output.print_success(&format!("已切换到主题: {}", theme_name));
            }
            None => {
                self.output.print_error(&format!("未知主题: {}", name));
                self.output.print_info("可用: dark, light, monokai, solarized, dracula, one-dark");
            }
        }
    }
    ...
    async fn cmd_output(&self, args: &[&str]) {
        if args.is_empty() {
            ...
            self.output.print_info(&format!("当前输出格式: {:?}", config.default_format));
            self.output.print_info("用法: /output <text|json|markdown|table>");
            return;
        }
    ...
    async fn cmd_verbose(&self) {
        ...
        if new_state {
            self.output.print_success("详细模式已开启 (显示 Token 统计和工具详情)");
        } else {
    ...
    async fn cmd_inspect(&self) {
        let (msg_count, tokens, token_limit, model, tools, skills, mcp, system_prompt) = self.agent.read_async(|a| Box::pin(async move {
            let (mc, tok) = a.context_stats().await;
            (
                mc,
                tok,
                ...
                a.tool_names().iter().map(|s| s.to_string()).collect::<Vec<_>>(),
                a.skill_names().iter().map(|s| s.to_string()).collect::<Vec<_>>(),
                a.mcp_server_names().iter().map(|s| s.to_string()).collect::<Vec<_>>(),
    ...
    async fn cmd_export(&self, args: &[&str]) {
        ...
        let (messages, model) = self.agent.read_async(|a| Box::pin(async move {
            let msgs = a.get_messages().await;
        ...
        match format {
            "json" => {
                ...
                match std::fs::write(&path, serde_json::to_string_pretty(&export_data).unwrap_or_default()) {
                    Ok(_) => self.output.print_success(&format!("已导出到: {}", path.display())),
            ...
            _ => {
                self.output.print_error(&format!("不支持的导出格式: {}", format));
    ...
    async fn cmd_profile(&self, args: &[&str]) {
            ...
            match manager.list() {
                Ok(list) => {
                    if list.is_empty() {
                        ...
                        self.output.print_info("使用 /profile create <名称> 创建档案。");
                    } else {
                        println!();
                        println!("╭─────────────────────────────────────────────────────────────╮");
                        println!("│                    配置档案 ({} 个)                           │", list.len());
                        println!("╰─────────────────────────────────────────────────────────────╯");
                        for p in &list {
                            let active = if p.active { " ★" } else { "" };
                            println!("  • {}{} — 模型: {} | 主题: {}", p.name, active, p.model, p.theme);
                        }
                        println!();
                    }
                }
        ...
        match args[0] {
            "create" => {
                if args.len() < 2 {
                    self.output.print_info("用法: /profile create <名称> [模型]");
                    return;
                }
                let name = args[1];
                ...
                match manager.save(&profile) {
                    Ok(()) => self.output.print_success(&format!("档案 '{}' 已创建", name)),
            ...
            "use" | "activate" => {
                ...
                match manager.activate(args[1]) {
                    Ok(profile) => {
                        ...
                        self.output.print_success(&format!("已激活档案 '{}' (模型: {})", profile.name, profile.model));
                    }
            ...
            "delete" | "rm" => {
                ...
                match manager.delete(args[1]) {
                    Ok(()) => self.output.print_success(&format!("档案 '{}' 已删除", args[1])),
                ...
                match manager.get(args[1]) {
                    Ok(profile) => {
                        let pairs: [(&str, &str); 7] = [
                            ...
                            ("系统提示词", profile.system_prompt.as_deref().unwrap_or("(未设置)")),
                            ("创建时间", &profile.created_at),
                            ("更新时间", &profile.updated_at),
                        ];
                        self.output.render_kv_table(&pairs);
                    }
                    Err(_) => self.output.print_error(&format!("档案 '{}' 不存在", args[1])),
                }
            }
            _ => {
                self.output.print_info("用法: /profile [list|create|use|delete|show]");
                self.output.print_info("  /profile              — 列出所有档案");
    ...
    async fn cmd_debug(&self, args: &[&str]) {
        // 使用静态检查器实例 (简化实现)
        if args.is_empty() {
            self.output.print_info("用法: /debug [on|off|stats|recent|clear]");
            self.output.print_info("  /debug on      — 开启 LLM 调用记录");
        ...
        match args[0] {
            "on" | "enable" => {
                self.output.print_success("调试记录已开启 (记录 LLM 请求/响应)");
                self.output.print_info("提示: 在 echo-agent.yaml 中设置 logging.inspect_llm_calls: true 可持久化开启");
            }
            "off" | "disable" => {
            ...
            "stats" => {
                // 显示当前 Agent 统计
                let (msg_count, tokens, token_limit, model) = self.agent.read_async(|a| Box::pin(async move {
                    let (mc, tok) = a.context_stats().await;
                    (mc, tok, a.config().get_token_limit(), a.model_name().to_string())
                })).await;
                ...
                self.output.render_kv_table(&pairs);
                self.output.print_info("提示: 使用 /inspect 查看更详细的 Agent 状态");
            }
            "recent" => {
                self.output.print_info("最近 LLM 调用记录:");
                self.output.print_info("(设置 logging.inspect_llm_calls: true 并在日志中查看)");
                self.output.print_info("使用 --verbose 启动可查看详细调用信息");
            }
            "clear" => {
                self.output.print_success("调试记录已清空");
            }
            _ => {
                self.output.print_error(&format!("未知的调试子命令: {}", args[0]));
                self.output.print_info("可用: on, off, stats, recent, clear");
            }
        }
    }
    ...
    async fn cmd_mode(&self, args: &[&str]) {
        if args.is_empty() {
            self.output.print_info(&format!("当前模式: {}", self.current_mode));
            self.output.print_info("可用模式: general, coding, research, data, writing");
        ...
        match agent_modes::AgentMode::from_str(args[0]) {
            Some(mode) => {
                ...
                self.agent.write_async(|a| Box::pin(async move {
            ...
            None => {
                ...
                self.output.print_info("可用: general, coding, research, data, writing");
            }
        }
    }
    ...
    async fn cmd_project(&self, args: &[&str]) {
        ...
        let pairs: [(&str, &str); 4] = [
            ...
            ("文件树摘要", if ctx.file_tree_summary.is_empty() { "(空)" } else { &format!("{} 行", ctx.file_tree_summary.lines().count()) }),
        ];
        self.output.render_kv_table(&pairs);
        ...
        if !ctx.instructions.is_empty() {
            ...
            for inst in &ctx.instructions {
                self.output.print_info(&format!("  • {} ({} 字符)", inst.source, inst.content.len()));
            }
    ...
    async fn cmd_undo(&self) {
        self.agent.write_async(|a| Box::pin(async move {
            ...
            if removed > 0 {
                ...
                tracing::info!("撤销了 {} 条消息 ({} -> {})", removed, original_len, original_len - removed);
            }
        })).await;
    ...
    async fn cmd_compact(&self) {
        ...
        self.output.print_info("正在轻量压缩 (滑动窗口, 保留近 6 轮)...");
    ...
    async fn cmd_think(&self, args: &[&str]) {
        ...
        match args[0].to_lowercase().as_str() {
            ...
            _ => {
                self.output.print_error(&format!("未知思考级别: {}", args[0]));
    ...
    async fn cmd_status(&self) {
        let (model, max_iter, msg_count, tokens, token_limit, tools, skills, mcp) = self.agent.read_async(|a| Box::pin(async move {
            let (mc, tok) = a.context_stats().await;
            (
                a.model_name().to_string(),
    ...
    async fn cmd_delegate(&self, args: &[&str]) {
        if args.is_empty() {
            ...
            self.output.print_info("  Agent 将创建一个隔离的子代理来执行该任务");
            return;
        }

        let task = args.join(" ");
        self.output.print_info(&format!("正在委派子代理执行: {}...", task.chars().take(60).collect::<String>()));

        let result = self.agent.read_async(|a| Box::pin(async move {
            a.delegate_task(&task).await
        })).await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                self.output.print_info("提示: 确保启用了 subagent 功能 (echo-agent.yaml: agent.enable_subagent: true)");
            }
        }
    }
    ...
    async fn cmd_search(&self, args: &[&str]) {
        ...
        if keyword == "/reindex" {
            match SessionSearchEngine::new() {
                Ok(engine) => match engine.reindex_all() {
                    Ok(count) => self.output.print_info(&format!("已重新索引 {} 个会话", count)),
                    Err(e) => self.output.print_error(&format!("重新索引失败: {}", e)),
                },
                Err(e) => self.output.print_error(&format!("搜索引擎初始化失败: {}", e)),
            }
            return;
        }
        ...
        match SessionSearchEngine::new() {
            Ok(engine) => match engine.search(&keyword, 20) {
                Ok(results) => {
                    if results.is_empty() {
                        self.output.print_info(&format!("未找到包含 '{}' 的会话", keyword));
                    } else {
                        println!();
                        println!("╭─────────────────────────────────────────────────────────────╮");
                        ...
                        println!("╰─────────────────────────────────────────────────────────────╯");
                        for r in &results {
                            let snippet = r.snippet.replace("<<", "\x1b[1;33m").replace(">>", "\x1b[0m");
                            println!("  • {} [{}] 模型: {}", r.session_name, r.session_id.chars().take(8).collect::<String>(), r.model);
                            println!("    {}", snippet);
                        }
                        println!();
                    }
    ...
    async fn cmd_doctor(&self) {
        ...
        // 附加 Agent 状态检查
        let (tools, skills, mcp_count) = self.agent.read(|a| {
            (a.tool_names().len(), a.skill_names().len(), a.mcp_server_names().len())
        }).await;
        ...
        if mcp_count > 0 {
            result.checks.push(format!("✅ 已连接 MCP: {} 个", mcp_count));
        } else {
    ...
    async fn cmd_cron(&self, args: &[&str]) {
        ...
        if args.is_empty() {
            self.output.print_info("用法: /cron <list|add|remove|enable|disable|run> [...]");
            return;
        }
        ...
        match args[0] {
                    ...
                    Ok(tasks) => {
                        if tasks.is_empty() {
                            ...
                            for t in &tasks {
                                ...
                                let next = t.next_run().map(|dt| dt.format("%m-%d %H:%M").to_string()).unwrap_or_else(|_| "invalid".to_string());
                                println!("  {} {} [{}] cron: {} | 下次: {}", status_icon, t.name, &t.id[..8], t.cron_expr, next);
                                println!("    prompt: {}", truncate_str(&t.prompt, 60));
                                if let Some(ref last) = t.last_run_at {
                                    println!("    上次执行: {}", last.chars().take(19).collect::<String>());
                                }
                            }
                            println!();
                        }
            ...
            "add" => {
                if args.len() < 3 {
                    self.output.print_info("用法: /cron add <cron表达式> <prompt>");
                    self.output.print_info("示例: /cron add \"0 9 * * *\" 生成日报");
                    return;
                }
                ...
                match store.add(task) {
                    Ok(()) => {
                        let next = cron::Schedule::from_str(&cron_expr)
                            ...
                            .unwrap_or_default();
                        self.output.print_success(&format!("定时任务已添加: {} (id: {}.., 下次执行: {})", name, &id[..8], next));
                    }
            ...
            "run" => {
                ...
                match store.get(&task_id) {
                    Ok(Some(task)) => {
                        self.output.print_info(&format!("正在执行任务: {}...", &task_id[..8]));
                        let prompt = task.prompt.clone();
                        let guard = self.agent.inner().read().await;
                        let result = guard.chat(&prompt).await;
                        match result {
                            Ok(answer) => {
                                self.output.print_success(&format!("任务完成: {}", truncate_str(&answer, 200)));
                                let _ = store.update_last_run(&task_id, &answer.chars().take(500).collect::<String>());
                            }
    ...
    fn cmd_trace(&self) {
        ...
        if trace.is_empty() {
            self.output.print_info("暂无 trace 数据（请先进行一次对话）");
            return;
        }

        println!();
        println!("{}", Color::Cyan.paint("╭─ 执行时间线 ─────────────────────────────────────────────────╮"));
        let mut prev_ms: u64 = 0;
        for entry in &trace {
            let delta = if prev_ms > 0 { entry.elapsed_ms - prev_ms } else { 0 };
