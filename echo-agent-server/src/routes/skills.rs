...
pub async fn load_skills_from_dir(
    ...
    match state
        .connection
        .agent
        .write_async(|agent| Box::pin(async move { agent.load_skills_from_dir(&path).await }))
        .await
    {
        Ok(loaded_skills) => {
            // 返回加载后的技能列表
...
pub async fn upload_skills(
    ...
    match state
        .connection
        .agent
        .write_async(|agent| Box::pin(async move { agent.load_skills_from_dir(&upload_dir).await }))
        .await
    {
        Ok(loaded_skills) => {
            let skills: Vec<SkillInfo> = state
